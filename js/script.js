...
const selectiveThreeCategories = ["分子动理论", "气体", "热力学定律", "原子结构", "原子核", "波粒二象性", "视频课程"];
const textbookCategoryMap = {
  ...
  selective3: selectiveThreeCategories,
};
...
const categoryIcons = {
  ...
  "分子动理论": `<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="7" cy="8" r="2"/><circle cx="16" cy="7" r="1.6"/><circle cx="14" cy="16" r="2"/><path d="M9 9l4 5M14 8l-5 7M5 17h4"/></svg>`,
  "气体": `<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="5" y="5" width="14" height="14" rx="3"/><circle cx="9" cy="10" r="1"/><circle cx="14" cy="8" r="1"/><circle cx="16" cy="15" r="1"/><path d="M9 10l4-2M14 8l2 7"/></svg>`,
  "热力学定律": `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 14a4 4 0 1 0 8 0c0-3-4-5-4-9 0 4-4 6-4 9Z"/><path d="M5 20h14"/></svg>`,
  "原子结构": `<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="2"/><ellipse cx="12" cy="12" rx="8" ry="3"/><ellipse cx="12" cy="12" rx="8" ry="3" transform="rotate(60 12 12)"/><ellipse cx="12" cy="12" rx="8" ry="3" transform="rotate(-60 12 12)"/></svg>`,
  "原子核": `<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="10" cy="11" r="2"/><circle cx="14" cy="11" r="2"/><circle cx="12" cy="15" r="2"/><path d="M5 19h14"/></svg>`,
  "波粒二象性": `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 12c2-5 4 5 6 0s4-5 6 0 4 5 6 0"/><circle cx="18" cy="7" r="2"/></svg>`,
...
function inferTextbookKey(raw) {
  ...
  if (String(raw.chapter || "").startsWith("选择性必修三")) return "选择性必修三";
  return "必修一";
}
...
function renderSupplementDiagram(rawId = "") {
  ...
  if (id.includes("wave_particle") || id.includes("photoelectric") || id.includes("de_broglie") || id.includes("matter_wave") || id.includes("uncertainty") || id.includes("compton") || id.includes("quantum")) {
    return `<svg class="mini-diagram motion-diagram wave-particle-diagram" viewBox="0 0 260 180"><defs>${svgArrow()}</defs><path class="trace" d="M34 92 C52 40 74 144 92 92 S132 40 150 92 190 144 208 92"/><circle class="moving-dot" cx="204" cy="92" r="10"/><path class="accent-line" d="M36 52h68" marker-end="url(#miniArrow)"/><path class="speed-arrow" d="M204 92h34" marker-end="url(#miniArrow)"/><text x="46" y="38">波动性</text><text x="176" y="76">粒子性</text><text x="70" y="154">E = hν，p = h / λ</text></svg>`;
  }
  if (id.includes("nuclear") || id.includes("radioactive") || id.includes("decay") || id.includes("fission") || id.includes("fusion") || id.includes("mass_defect") || id.includes("binding_energy") || id.includes("half_life")) {
    return `<svg class="mini-diagram motion-diagram nuclear-diagram" viewBox="0 0 260 180"><defs>${svgArrow()}</defs><circle class="moving-dot" cx="112" cy="88" r="14"/><circle class="moving-dot small" cx="138" cy="78" r="10"/><circle class="moving-dot small" cx="136" cy="106" r="9"/><circle class="moving-dot small" cx="96" cy="108" r="9"/><path class="accent-line" d="M154 88h54" marker-end="url(#miniArrow)"/><path class="speed-arrow" d="M112 88 72 50" marker-end="url(#miniArrow)"/><path class="field-line" d="M70 134 C104 152 154 152 196 128"/><text x="84" y="34">α / β / γ</text><text x="166" y="78">释放能量</text><text x="74" y="164">ΔE = Δmc²</text></svg>`;
  }
  if (id.includes("atomic") || id.includes("bohr") || id.includes("spectrum") || id.includes("energy_level") || id.includes("hydrogen") || id.includes("electron_transition") || id.includes("photoelectric")) {
    return `<svg class="mini-diagram motion-diagram atomic-diagram" viewBox="0 0 260 180"><defs>${svgArrow()}</defs><circle class="trace" cx="130" cy="90" r="54"/><ellipse class="field-line" cx="130" cy="90" rx="82" ry="28"/><ellipse class="field-line" cx="130" cy="90" rx="82" ry="28" transform="rotate(62 130 90)"/><ellipse class="field-line" cx="130" cy="90" rx="82" ry="28" transform="rotate(-62 130 90)"/><circle class="moving-dot" cx="130" cy="90" r="10"/><circle class="moving-dot small" cx="184" cy="90" r="5"/><path class="accent-line" d="M184 90 206 58" marker-end="url(#miniArrow)"/><text x="108" y="94">核</text><text x="178" y="50">跃迁</text><text x="76" y="160">hν = Em - En</text></svg>`;
  }
  if (id.includes("thermo") || id.includes("heat") || id.includes("adiabatic") || id.includes("entropy") || id.includes("engine") || id.includes("refrigerator") || id.includes("energy_conservation_heat")) {
    return `<svg class="mini-diagram motion-diagram thermo-diagram" viewBox="0 0 260 180"><defs>${svgArrow()}</defs><rect class="force-block" x="82" y="58" width="96" height="70" rx="12"/><path class="accent-line" d="M38 92h44" marker-end="url(#miniArrow)"/><path class="speed-arrow" d="M178 92h44" marker-end="url(#miniArrow)"/><path class="height-line" d="M130 128v32"/><path class="force-arrow" d="M130 58V28" marker-end="url(#miniArrow)"/><text x="44" y="82">Q</text><text x="192" y="82">W</text><text x="138" y="38">ΔU</text><text x="82" y="164">ΔU = Q + W</text></svg>`;
  }
  if (id.includes("gas") || id.includes("boyle") || id.includes("charles") || id.includes("gay_lussac") || id.includes("isothermal") || id.includes("isobaric") || id.includes("isochoric") || id.includes("ideal")) {
    return `<svg class="mini-diagram motion-diagram gas-diagram" viewBox="0 0 260 180"><defs>${svgArrow()}</defs><rect class="force-block" x="58" y="38" width="142" height="104" rx="10"/><path class="height-line" d="M200 38v104"/><path class="accent-line" d="M202 90h34" marker-end="url(#miniArrow)"/><circle class="moving-dot small" cx="88" cy="70" r="6"/><circle class="moving-dot small" cx="140" cy="60" r="5"/><circle class="moving-dot small" cx="166" cy="112" r="7"/><path class="speed-arrow" d="M88 70l34 20" marker-end="url(#miniArrow)"/><path class="speed-arrow" d="M166 112l-38 18" marker-end="url(#miniArrow)"/><text x="72" y="28">大量碰撞形成压强</text><text x="64" y="164">pV = nRT</text><text x="210" y="82">活塞</text></svg>`;
  }
  if (id.includes("molecular") || id.includes("brownian") || id.includes("diffusion") || id.includes("internal_energy") || id.includes("temperature_micro") || id.includes("molecule_force") || id.includes("thermal_motion")) {
    return `<svg class="mini-diagram motion-diagram molecular-diagram" viewBox="0 0 260 180"><defs>${svgArrow()}</defs><rect class="force-block" x="42" y="34" width="176" height="112" rx="14"/><circle class="moving-dot small" cx="82" cy="76" r="8"/><circle class="moving-dot small" cx="136" cy="62" r="6"/><circle class="moving-dot small" cx="174" cy="104" r="9"/><circle class="moving-dot small" cx="108" cy="122" r="5"/><path class="accent-line" d="M82 76l42-14" marker-end="url(#miniArrow)"/><path class="speed-arrow" d="M136 62l34 34" marker-end="url(#miniArrow)"/><path class="field-line" d="M62 130 C92 100 130 154 166 118 184 100 198 92 216 86"/><text x="58" y="28">大量分子无规则运动</text><text x="70" y="164">温度越高，平均动能越大</text></svg>`;
  }
  if (id.includes("alternating") || id.includes("ac_") || id.includes("rms") || id.includes("transformer") || id.includes("transmission") || id.includes("sine_current") || id.includes("peak_value") || id.includes("frequency")) {
    return `<svg class="mini-diagram motion-diagram ac-diagram" viewBox="0 0 260 180"><defs>${svgArrow()}</defs><path class="track-line" d="M34 92H226"/><path class="height-line" d="M54 34v116"/><path class="trace" d="M54 92 C72 30 94 30 112 92 S152 154 170 92 208 30 226 92"/><path class="accent-line" d="M54 34h34"/><path class="accent-line" d="M54 150h34"/><path class="span-line" d="M112 160h58"/><path class="speed-arrow" d="M170 160h42" marker-end="url(#miniArrow)"/><circle class="moving-dot small" cx="112" cy="92" r="5"/><text x="30" y="42">峰值</text><text x="96" y="174">周期 T</text><text x="176" y="50">i = Im sinωt</text><text x="178" y="112">方向周期变化</text></svg>`;
  }
